...
const ProductForm = (props) => {
    ...
    const [rating, setRating] = useState(product.featured);
    ...
    const isCheckedFeather = useMemo(() => {
        return rating > 8 ? true : false;
    } , [rating])

    const [expirationDateError, setExpirationDateError] = useState(false);

    const onChangeExpirationDate = (val) => {
        ...
        <Form onSubmit={onSubmit}>
            ...
            <FormGroup>
                ...
                <Input
                    ...
                    onChange={({target}) => onChangeExpirationDate(target.value)}
                />
                {expirationDateError && <FormFeedback>If a product has an expiration date it must expire not less than 30 days since
                    now</FormFeedback>}
            </FormGroup>
            <FormGroup>
            ...
            <FormGroup check>
                <Label check>
                    <Input type="checkbox" checked={isCheckedFeather}
                           onChange={() => {return;}}
                    />{' '}
                    Featured
                </Label>
